...
Core Python code lives in `src/edenfintech_scanner_bootstrap/`. `cli.py` exposes the command surface, `pipeline.py` runs the stage orchestration, `scoring.py` holds deterministic valuation and PCS math, `validation.py` checks vendored assets, `regression.py` runs fixture-based checks, and `assets.py` centralizes repository paths. Repository data is under `assets/`: `contracts/` for stage definitions, `methodology/` for source-of-truth docs and schemas, `rules/` for the canonical rulebook, and `fixtures/regression/` for regression inputs plus `manifest.json`. Tests live in `tests/`.
...
`PYTHONPATH=src` is required in a fresh checkout because the package is not installed into the environment by default. Run `validate-assets` after editing anything in `assets/`, `run-regression` before changing contracts or fixtures, `validate-scan-input` when you touch the scan input contract, and `run-scan` when you change stage orchestration or scoring logic.
