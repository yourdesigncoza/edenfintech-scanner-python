...
def _build_synthesis_system_prompt(sector_knowledge: dict | None = None) -> str:
    return (
        ...
        "DEDUPLICATION: The catalysts array must not contain duplicate entries. If a Gemini-sourced "
        "catalyst with source citations duplicates an analyst-written catalyst, keep only the version "
        "with the source citation. The catalyst_stack array (structured with type/description) is "
